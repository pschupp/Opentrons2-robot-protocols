...
def supernatant_removal(vol,src,dest,pip,xdir,bead_transfer='F',is_waste='F'): 
	...
	asp_ctr = 0 
	while tvol > 250:
		pip.aspirate(250,src.bottom().move(types.Point( x=-1*xdir,y=0,z=.8))) 
		...
		asp_ctr=asp_ctr+1
	if bead_transfer=='T':
		pip.aspirate(tvol,src.bottom().move(types.Point(x=0,y=0,z=0.1))) 
	else:	
		pip.aspirate(tvol,src.bottom().move(types.Point(x=-1*xdir,y=0,z=.8))) 
...
def run(protocol):
	...
	reagentPlate=protocol.load_labware('usascientific_12_reservoir_22ml','2','Reagent Plate') 
	...
	sampleTransfer2=[elutionPlate['A'+str(i)] for i in range(2,numberColumns*2+1,2)]
	print(sampleTransfer2)
	sampleRNA=[elutionPlate['A'+str(i)] for i in range(2,numberColumns*2+1,2)]
	print(sampleRNA)
	print('\n')
	print('Mag bead here:')
	...
	for well,reagent,tip in zip(sample,magBead,tips2): 
		...
		init_well_mix(2,reagent,volumeMix,p300,xdir=xdir) 
		p300.aspirate(30,reagent.bottom()) 
		p300.dispense(40,well.top(-10)) 
		init_well_mix(1,well,200,p300,xdir,1430) 
		p300.aspirate(10,well.top())
		p300.return_tip()
		i=i+1
...
#	protocol.delay(minutes=11) 
	...
	wash_step(sample,ethanol4,1200,3,tips3,tips3,p300,zheight=15,xdir=xdir*-1)
	magdeck.engage(height=magheight)
	protocol.delay(minutes=3)
	wash_step_1_5(sample,ethanol4,1200,5,tips3,tips3,p300,zheight=15,is_etoh='T')
	protocol.delay(minutes=4)

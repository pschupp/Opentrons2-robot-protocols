...
p300Location = ["5", "9", ][:p300tipNumber]
print(
...
def run(protocol):
    ...
    mastermix_transfer(p20, tips_1, pcre3_mm, warm_sample, 8.5)
    protocol.pause("Run thermocycler after spinning down sample")
    
    thermocycler.close_lid()
    profile = [
        {'temperature': 98, 'hold_time_seconds': 10},
        {'temperature': 65, 'hold_time_seconds': 20},
        {'temperature': 72, 'hold_time_seconds': 30},
    ]

    thermocycler.set_block_temperature(98, hold_time_seconds=30, block_max_volume=35)
    thermocycler.execute_profile(steps=profile, repetitions=15, block_max_volume=35)
    thermocycler.set_block_temperature(72, hold_time_seconds=60, block_max_volume=35)
    thermocycler.set_block_temperature(10, block_max_volume=35)
    thermocycler.open_lid()

    protocol.comment("spin down samples and place on magnet")
    protocol.comment("Part 3 complete!")
    protocol.comment("On to the final clean up!")

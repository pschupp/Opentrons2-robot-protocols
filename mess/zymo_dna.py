...
def volume_calc(volume, reservoir, numberColumns, overageFactor):
	...
	if volume>1800: sys.exit('volume for one of the reagents is greater than 1800!')
...
def supernatant_removal(vol,src,dest, pip, xdir, bead_transfer='F', is_waste='F'): # magnet is engaged, need to change x depending on odds or evens
	...
	if bead_transfer=='T':
		...
		pip.aspirate(tvol, src.bottom().move(types.Point(x=-1.2*xdir, y=0, z=.8))) # aspirate remaining volume 
...
def run(protocol):
	...
	elutionPlate=protocol.load_labware('nest_96_wellplate_100ul_pcr_full_skirt', '9', 'DNA Elution Plate') 

...
metadata = {
    "protocolName": "Swift RNAseq Library kit",
...
def p20_mix(reps, vol, loc, pip, xdir=None, high_mix="F"):
    if high_mix == "T":
        # if xdir is None:
        loc1 = loc.bottom(0.7)
        loc2 = loc.bottom(6)
...
def p20_supernatant_removal(
    ...
    while tvol > 17:
        pip.aspirate(
            17, src.bottom().move(types.Point(x=-0.5 * xdir, y=0, z=0.5))
        ...
        asp_ctr = asp_ctr + 1
    pip.aspirate(
        tvol, src.bottom().move(types.Point(x=-0.5 * xdir, y=0, z=0.1))
...
def p300_supernatant_removal(vol, src, dest, pip, xdir, super_type="sample"):
    ...
    if super_type == "etoh_waste":
        ...
        while tvol > 80:
            pip.aspirate(
                80, src.bottom().move(types.Point(x=-1 * xdir, y=0, z=6))
            )  # aspirating away from the side with the magnests
            pip.aspirate(80, src.bottom().move(types.Point(x=-0.6 * xdir, y=0, z=0.7)))
            ...
            asp_ctr = asp_ctr + 1
        pip.aspirate(
            80, src.bottom().move(types.Point(x=-0.4 * xdir, y=0, z=0.3))
        )  # aspirate remaining volume
        dvol = (
            30 * asp_ctr + tvol
        )  # calculate volume that was just aspirated and add to it 20 ul for every time there was pipetting
    ...
    if (
        ...
        pip.flow_rate.dispense = 150
        pip.aspirate(
            80, src.bottom().move(types.Point(x=-1 * xdir, y=0, z=2.5))
        )  # aspirating away from the side with the magnests
        pip.aspirate(10, src.top())
        pip.dispense(300, dest)
        pip.blow_out()
...
def mastermix_transfer(pip, tips, reagent_well, sample_well, transfer_vol):
    ...
    for tip, r_well, s_well in zip(tips, reagent_well, sample_well):
        ...
        while tvol > 18:
            ...
            tvol = tvol - 18
            asp_ctr = asp_ctr + 1
        pip.aspirate(tvol, r_well.bottom(0.5))
...
def SPRI_clean_up_a(
    pip, tips, sample, bead_vol, te_vol, transfer_vol, transfer_loc, xdirs, mag_bead
):
    i = 1
    for tip, beads, mag_samp, xdir in zip(tips, mag_bead, sample, xdirs):
        if i == 1:
            pip.flow_rate.aspirate = 100
            pip.flow_rate.dispense = 150
            pip.pick_up_tip(tip)
            p300_mix(10, 220, beads, pip, xdir=xdir, high_mix="T")
        if not pip.hw_pipette["has_tip"]:
            pip.pick_up_tip(tip)
        p300_mix(3, 100, beads, pip, xdir=xdir, high_mix="T")
        ...
        pip.flow_rate.aspirate = 70
        pip.flow_rate.dispense = 50
        p300_mix(8, 80, mag_samp, pip, xdir=xdir)
...
def SPRI_clean_up_bb(
    ...
    for tip, mag_samp, xdir in zip(tips, sample, xdirs):
        pip.pick_up_tip(tip)
        pip.aspirate(20, mag_samp.bottom().move(types.Point(x=-0.7 * xdir, y=0, z=0.2)))
        pip.drop_tip()
...
def SPRI_clean_up_c(
    ...
    for tip, te, mag_samp, xdir in zip(tips, low_edta_te, sample, xdirs):
        pip.pick_up_tip(tip)
        if pip == "p300":
            pip.aspirate(te_vol, te)
            pip.dispense(
                te_vol, mag_samp.top().move(types.Point(x=0.7 * xdir, y=0, z=-6))
            )
        else:
            tvol = te_vol
            asp_ctr = 0
        ...
        if pip == "p300":
            p300_mix(10, te_vol, mag_samp, pip, xdir=xdir)
        else:
            if te_vol < 20:
                p20_mix(10, te_vol, mag_samp, pip, xdir=xdir)
            else:
...
def SPRI_clean_up_d(
    ...
    for tip, mag_samp, trans_loc, xdir in zip(tips, sample, transfer_loc, xdirs):
        ...
        if pip == "p300":
            ...
            p20_supernatant_removal(transfer_vol, mag_samp, trans_loc, pip, xdir=xdir)
...
reverse_transcription_mm_volume = volume_calc(
    6.5, "96well100", numberColumns, overageFactor
)
...
print("Mag Beads")
mag_bead_volume = volume_calc(
    (SPRI_1_vol + SPRI_2a_vol + SPRI_2b_vol + SPRI_3_vol + SPRI_4_vol),
    "96well2000",
    numberColumns,
    overageFactor,
)
...
print("Low EDTA TE")
low_edta_te_volume = volume_calc(
    30 + 12 + 52 + 17 + 22 + 22, "96well2000", numberColumns, overageFactor
)


def run(protocol):
    mag_max, cold_max = 3, 4
    ...
    rt_reagents = protocol.load_labware("usascientific_12_reservoir_22ml", "2")
    print("rt_reagents 96-well plate in slot 3")
    cold_plate = protocol.load_labware("nest_96_wellplate_100ul_pcr_full_skirt", "3")
    ...
    [
        ...
        for i in range(0, numberColumns * 30, numberColumns)
    ]
    print(TIPS_1)
    print(TIPS_2)
    print(TIPS_30)
    ...
    fragmentation_mm, reagentCounter = reagent_assign(
        fragmentation_mm_volume, cool_reagents, reagentCounterInit
    )
    reverse_transcription_mm, reagentCounter = reagent_assign(
        reverse_transcription_mm_volume, cool_reagents, reagentCounter
    )
    adaptase_mm, reagentCounter = reagent_assign(
        adaptase_mm_volume, cool_reagents, reagentCounter
    )
    extension_mm, reagentCounter = reagent_assign(
        extension_mm_volume, cool_reagents, reagentCounter
    )
    ligation_mm, reagentCounter = reagent_assign(
        ligation_mm_volume, cool_reagents, reagentCounter
    )
    pcr_mm, reagentCounter = reagent_assign(
        pcr_mm_volume, cool_reagents, reagentCounter
    )
    mag_bead, reagentRTCounter = reagent_assign(
        mag_bead_volume, rt_reagents, reagentRTCounterInit
    )
    ethanol, reagentRTCounter = reagent_assign(
        ethanol_volume, rt_reagents, reagentRTCounter
    )
    low_edta_te, reagentRTCounter = reagent_assign(
        low_edta_te_volume, rt_reagents, reagentRTCounter
    )
    # TODO finish this

    mag_samps, cold_samps, col_numss, xdirss = [], [], [], []
    for ns in range(max(mag_max, cold_max)):
        col_nums = list(
        ...
        if ns <= cold_max:
            cold_samps.append([cold_plate["A" + str(i)] for i in col_nums])
        if ns <= mag_max:
    ...
    protocol.comment("Engaging Magnetic Module and incubate for 4 minutes.")
    protocol.delay(minutes=4)
    # Transfer supernatant
    protocol.comment("Transfer Supernatent from Magnet plate to Cold plate")
    for cold_well, mag_well, tip, xdir in zip(
        cold_samps[0], mag_samps[0], tips_1, xdirss[0]
    ):
        p20.pick_up_tip(tip)
        print("THIS", type(mag_well), xdir)
        p20_supernatant_removal(14.5, mag_well, cold_well, p20, xdir=xdir)
        p20.drop_tip()
    magdeck.disengage()
    protocol.comment("lin4 717")
    ...
    mastermix_transfer(p20, tips_2, reverse_transcription_mm, cold_samps[0], 6.5)
    protocol.pause("Run Thermocycler off deck")
    protocol.comment("Line 726")
    """
    ...
    #   SPRI_clean_up_bb(p20,tips_3,mag_samps[0],waste,SPRI_1_vol,12,10,cold_samps[1],xdir,mag_bead,ethanol)
    protocol.comment("Line 771")
    ...
    protocol.comment("Line 788")

    # Incubate -> Engage -> Incubate
    protocol.comment("Incubating for 3 minutes.")
    protocol.delay(minutes=3)
    protocol.comment("Engaging Magnetic Module and incubating for  minutes.")
    magdeck.engage()
    protocol.delay(minutes=4)
    ...
    protocol.comment("Line 811")

    # Adaptase; Preheat thermocycler to 95C; Denature 10ul eluate @95C w/lid temp on for 2 minutes;
    # Move samples to cold block
    ...
    mastermix_transfer(p20, tips_6, adaptase_mm, cold_samps[1], 11)
    protocol.pause("Run Thermocycler off deck")
    protocol.comment("Line 821")
    protocol.comment("Return plate to cold block")
    protocol.comment("Replace p20 tips!")

    # Extension
    mastermix_transfer(p20, tips_1, extension_mm, cold_samps[1], 23.5)
    protocol.pause("Run Thermocycler off deck")
    protocol.comment("Line 840")
    ...
    #   SPRI_clean_up_bb(p20,tips_2,mag_samps[1],waste,SPRI_2a_vol,52,50,cold_samps[1],xdir,mag_bead,ethanol)
    print("You made it to line...", lineno())
    ...
    SPRI_clean_up_b(
        p300,
        [TIPS_14, TIPS_15, TIPS_16, TIPS_17],
        mag_samps[1],
        waste,
        SPRI_2b_vol,
        17.5,
    ...
    #   SPRI_clean_up_bb(p20,tips_3,mag_samps[1],waste,SPRI_2b_vol,17,15,cold_samps[2],xdir,mag_bead,ethanol)
    protocol.comment("Letting beads dry for 30 seconds.")
    protocol.delay(seconds=30)
    ...
    magdeck.disengage()
    protocol.comment("Line 991")

  

    # Ligation
    mastermix_transfer(p20, tips_6, ligation_mm, cold_samps[2], 15.5)
    protocol.pause("Run Thermocycler off deck")
    protocol.comment("Line 989")
    print("You made it to line...", lineno())
    ...
    protocol.comment("Line 1010")
    magdeck.engage(magheight)
    protocol.delay(minutes=6)
    SPRI_clean_up_b(
        p300,
        [TIPS_19, TIPS_20, TIPS_21, TIPS_1],
        mag_samps[2],
        waste,
        SPRI_3_vol,
        22.5,
    ...
    #   SPRI_clean_up_bb(p20,tips_1,mag_samps[2],waste,SPRI_3_vol,22.5,20.5,cold_samps[2],xdir,mag_bead,ethanol)
    protocol.comment("Letting beads dry for 30 seconds.")
    protocol.delay(seconds=30)
    protocol.comment("Line 1030")
    magdeck.disengage()
    SPRI_clean_up_c(
        p20,
        tips_2,
    ...
    magdeck.disengage()
    protocol.comment("Line 1066")

    # Transfer PCR master mix to sample
    mastermix_transfer(p20, tips_4, pcr_mm, cold_samps[2], 25.5)
    protocol.pause("Run Thermocycler off deck")
    protocol.comment("Line 1112")
    protocol.comment("Return plate to magnetic module")
    ...
    # Incubate -> Engage -> Incubate
    protocol.delay(minutes=5)
    protocol.comment("Line 1130")
    magdeck.engage(magheight)
    protocol.delay(minutes=6)
    SPRI_clean_up_b(
        p300,
        [TIPS_3, TIPS_4, TIPS_5, TIPS_6],
        mag_samps[2],
        waste,
        SPRI_4_vol,
        22.5,
    ...
    #   SPRI_clean_up_bb(p20,tips_5,mag_samps[2],waste,SPRI_4_vol,22.5,20.5,cold_samps[3],xdir,mag_bead,ethanol)
    protocol.comment("Letting beads dry for 30 seconds.")
    protocol.delay(seconds=30)
    protocol.comment("Line 1150")
    magdeck.disengage()
    SPRI_clean_up_c(
        p20,
        tips_6,

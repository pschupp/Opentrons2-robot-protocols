...
def run(protocol):
    ...
    sampleRNA = [
        elutionPlate["A" + str(i)]
        for i in range(startWell, startWell + numberColumns , 1)
    ]
    '''
    print(sampleTransfer2)
    sampleRNA = [
    ...
    for well, tip, tret, waterWell in zip(sample, tips8, tips8, water):
        p300.pick_up_tip(tip)
        p300.aspirate(57, waterWell)
        p300.dispense(80, well)
        elute_mix(2, well, 50, p300, xdir=xdir)
        p300.blow_out()
        p300.drop_tip(tret)
    ...
    for src, dest, tip in zip(sample, sampleRNA, tips9):
        p300.pick_up_tip(tip)
        p300.aspirate(50, src.bottom().move(types.Point(x=1, y=0, z=0.5)))

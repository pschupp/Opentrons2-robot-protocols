...
def p300_mix(reps, vol, loc, pip, xdir, high_mix="F"):
    if high_mix == "T":
        ...
        loc1 = loc.bottom().move(types.Point(x=0, y=0, z=1))
        loc2 = loc.bottom().move(types.Point(x=1.2 * xdir, y=0, z=3))
    pip.aspirate(5, loc1)
    mvol = vol - 10
    for _ in range(reps - 1):
        pip.aspirate(mvol, loc1)
...
def p300_supernatant_removal(vol, src, dest, pip, xdir, super_type="sample"):
    ...
    if (
        ...
        pip.flow_rate.aspirate = 5
        pip.aspirate(
            100, src.bottom().move(types.Point(x=-1 * xdir, y=0, z=2.5))
        )  # aspirating away from the side with the magnests
        pip.aspirate(10, src.top())
        pip.dispense(300, dest)
        pip.blow_out()
...
startWell = 1
SPRI_1_vol = 16
SPRI_2_vol = 31  # more for dual index kit and non ffpe
...
fs1fs2e1_mm_volume = volume_calc(15, "96well100", numberColumns, overageFactor)
print("RS Master Mix")
...
ps_volume = volume_calc(48 + 30, "96well2000", numberColumns, overageFactor)
...
def run(protocol):
    ...
    fs1fs2e1_mm, reagentCounter = reagent_assign(
        fs1fs2e1_mm_volume, mm_plate, reagentCounterInit
    )
    ...
    print("\n")
    print("FS1/FS2/E1 mastermix here:")
    print(fs1fs2e1_mm_volume["reservoir.volume"])
    print(unique(fs1fs2e1_mm))
    print("\n")
    print("RS mm here:")
    ...
    # Transfer FS1/FS2/E1
    protocol.comment("pre-warm mm 3min at 42C")
    mastermix_transfer(p20, tips_1, fs1fs2e1_mm, mag_samps[0], 15.5)
    protocol.comment("pre-warm mm 3min at 42C")
    protocol.pause("Incubate sample on thermocycler for 15min - 1hr at 42C")
    protocol.comment("Line 891")
    ...
    # Transfer RS
    mastermix_transfer(p20, tips_2, rs_mm, mag_samps[0], 5.5)
    tempdeck.set_temperature(37)
    protocol.pause("Incubate sample on thermocycler for 10min at 95C")
    tempdeck.deactivate()
    protocol.comment("Line 885")

    # Transfer USS (UMI second strand mix)
    protocol.comment("Thaw USS at 37C")
    mastermix_transfer(p20, tips_3, uss_mm, mag_samps[0], 10.5, mix_type="thick_mix")
    protocol.pause(
    ...
    protocol.comment("spin down samples")
    protocol.comment("Line 894")
    print("You made it to line...", lineno())

    # Transfer SS2/E2
    protocol.comment("Keep mastermix at RT")
    mastermix_transfer(p20, tips_4, ss2e2_mm, mag_samps[0], 5.5)
    protocol.pause("Incubate sample on thermocycler for 15min at 25C")
    protocol.comment("Spin down samples")
    protocol.comment("Safe Stopping point, store at -20")
    protocol.comment("Line 903")
    protocol.comment("Return plate to magnet")
    ...
    # SPRI_clean_up_a(p300, TIPS_1, mag_samps[0], 16.5, xdirss[0], mag_bead1, mag_bead1_volume)
    ...
    SPRI_clean_up_b(p300, [TIPS_2, TIPS_3], mag_samps[0], waste, xdirss[0], ethanol)
    # protocol.delay(minutes= 4)
    magdeck.disengage()

    SPRI_clean_up_c(p300, TIPS_4, mag_samps[0], xdirss[0], elution_buffer, 40.5)
    protocol.delay(minutes=2)

    SPRI_clean_up_d(p300, TIPS_5, mag_samps[0], xdirss[0], ps, 48.5)
    ...
    SPRI_clean_up_e(
        p300, [TIPS_6, TIPS_7, TIPS_8, TIPS_9], mag_samps[0], waste, xdirss[0], ethanol
    )
    protocol.delay(minutes=4)  # let beads dry
    protocol.comment("Place plate with idexes in warm_plate position")
    magdeck.disengage()

    SPRI_clean_up_c(p300, TIPS_10, mag_samps[0], xdirss[0], elution_buffer, 20.5)
    protocol.delay(minutes=2)
    magdeck.engage(magheight)
    protocol.delay(minutes=5)

    SPRI_clean_up_f(p20, tips_6, mag_samps[0], 17.5, warm_samps[0], xdirss[0])
    ...
    # PCR
    protocol.comment("Thaw indxes and pcr mix at RT")
    protocol.comment("Thaw enzyme on ice")
    mastermix_transfer(p20, tips_1, pcre3_mm, warm_samps[0], 8.5)
    protocol.pause("Run thermocycler after spinning down sample")
    protocol.comment("spin down samples and place on magnet")

    # Bead clean up
    magdeck.disengage()
    SPRI_clean_up_a(
        p300, TIPS_11, mag_samps[0], 31.5, xdirss[0], mag_bead2, mag_bead2_volume
    )
    ...
    SPRI_clean_up_b(p300, [TIPS_12, TIPS_13], mag_samps[0], waste, xdirss[0], ethanol)
    # protocol.delay(minutes= 4)
    magdeck.disengage()

    SPRI_clean_up_c(p300, TIPS_14, mag_samps[0], xdirss[0], elution_buffer, 30.5)
    protocol.delay(minutes=2.0)

    SPRI_clean_up_d(p300, TIPS_15, mag_samps[0], xdirss[0], ps, 30.5)
    ...
    SPRI_clean_up_e(
        p300,
        [TIPS_16, TIPS_17, TIPS_18, TIPS_19],
        mag_samps[0],
        waste,
        xdirss[0],
        ethanol,
    ...
    """
    SPRI_clean_up_c(p300, TIPS_1, mag_samps[0], xdirss[0], elution_buffer, 20.5)
    protocol.delay(minutes=2)
    magdeck.engage(magheight)
    protocol.delay(minutes=5)

    SPRI_clean_up_f(p20, tips_2, mag_samps[0], 17.5, warm_samps[0], xdirss[0])

    protocol.comment("Well done lad!")

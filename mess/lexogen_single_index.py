...
startWell = 1
SPRI_1_vol = 16
SPRI_2_vol = 30  # more for dual index kit
...
def run(protocol):
   ...
   # numberColumns = 2
    ...
    # Transfer FS1
    print(mag_samps)
    mastermix_transfer(p20, tips_1, fs1_mm, mag_samps[0], 5.5)
    protocol.pause(
        "Incubate sample on thermocycler for 3min at 85C, then cool down to 42C"
    )

    protocol.comment("Line 860")
    ...
    # Transfer FS2/E1
    mastermix_transfer(p20, tips_2, fs2e1_mm, warm_samps[0], 10.5)
    tempdeck.deactivate()
    protocol.pause("Incubate sample on thermocycler for 15min at 42C")
    protocol.comment("Line 874")
    protocol.comment("spin down samples")
    ...
    # Transfer RS
    mastermix_transfer(p20, tips_3, rs_mm, mag_samps[0], 5.5)
    ...
    mastermix_transfer(p20, tips_4, uss_mm, mag_samps[0], 10.5, mix_type="thick_mix")
    protocol.pause(
    ...
    mastermix_transfer(p20, tips_5, ss2e2_mm, mag_samps[0], 5.5)
    ...
    # Bead clean up
    magdeck.disengage()
    mastermix_transfer(p20, tips_6, mag_bead1, mag_samps[0], 16.5, mix_type="bead_mix", )
    ...
    SPRI_clean_up_d(p300, TIPS_5, mag_samps[0], xdirss[0], ps, 56.5)
    ...
    SPRI_clean_up_f(p20, tips_1, mag_samps[0], 17.5, warm_samps[0], xdirss[0])
    ...
    mastermix_transfer(p20, tips_2, pcre3_mm, warm_samps[0], 8.5)
    ...
    magdeck.disengage()
    SPRI_clean_up_a(
        p300, TIPS_11, mag_samps[0], 30.5, xdirss[0], mag_bead2, mag_bead2_volume
    )
    ...
    SPRI_clean_up_c(p300, TIPS_20, mag_samps[0], xdirss[0], elution_buffer, 20.5)
    protocol.delay(minutes=2)
    magdeck.engage(magheight)
    protocol.delay(minutes=5)

    SPRI_clean_up_f(p20, tips_3, mag_samps[0], 17.5, warm_samps[0], xdirss[0])

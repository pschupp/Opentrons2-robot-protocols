...
def init_well_mix(reps, loc, vol,pip, xdir): # mixing on both sides of the well, not used while magnet is engaged!
	...
	loc4 = loc.bottom().move(types.Point(x=-1.3*xdir, y=-.8, z=0.3))
...
def supernatant_removal(vol,src,dest, pip, xdir, bead_transfer='F', is_waste='F'): # magnet is engaged, need to change x depending on odds or evens
	...
	while tvol > 250:
		...
		asp_ctr=asp_ctr+1
	if bead_transfer=='T':
		pip.aspirate(tvol, src.bottom().move(types.Point(x=0, y=0, z=0.2))) # aspirate remaining volume	
	else:	
		pip.aspirate(tvol, src.bottom().move(types.Point(x=-1*xdir, y=0, z=.4))) # aspirate remaining volume 
...
def wash_step(sample, src, vol, mtimes, tips, usedtips, pip,zheight,xdir) :	# magnet not engaged
	for well, tip, tret, srcWell in zip(sample, tips, usedtips, src): 
		...
		while tvol > 250: 
			...
			pip.dispense(280, well.top().move(types.Point(x=1.3*xdir, y=0, z=-10)))
...
def run(protocol):
	...
	magheight = 6.75 
	magplate = magdeck.load_labware('nest_96_wellplate_2ml_deep', 'Sample Plate') 
	...
	reagentPlate=protocol.load_labware('nest_96_wellplate_2ml_deep', '3', 'Reagent Plate') 
...
#	magBead,reagentCounter=reagent_assign(magBeadVolume,reagentPlate,reagentCounter)
	...
	sampleDNA=[elutionPlate['A'+str(i)] for i in range(startWell,2*numberColumns+startWell,2)]
	sampleRNA=[nextRNAPlate['A'+str(i)] for i in range(startWell,2*numberColumns+startWell,2)]
	magdeck.disengage()
...
#	protocol.delay(minutes=30) # only use delay if you are mixing off deck                                                                                                                      ) #this is for protocols mixing off deck
	...
	"""	
	protocol.comment('Allowing beads to air dry for 9 minutes.')

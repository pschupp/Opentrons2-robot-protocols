...
def run(protocol):
    ...
    mag_bead1, reagentCounter = reagent_assign(
        mag_bead1_volume, mm_plate, reagentCounterInit
    )
    ...
    """
    SPRI_clean_up_a(
        p300, TIPS_1, mag_sample, 35.5, xdirss, mag_bead2, mag_bead2_volume
    )"""
    ...
    SPRI_clean_up_b(p300, [TIPS_2, TIPS_3], mag_sample, waste, xdirss, ethanol) #the second part of this function is not currently used so TIPS_2 can be used by the next function
    #protocol.delay(minutes= 4)
    magdeck.disengage()

    SPRI_clean_up_c(p300, TIPS_3, mag_sample, xdirss, elution_buffer, 30.5)
    protocol.delay(minutes=2.0)

    SPRI_clean_up_d(p300, TIPS_4, mag_sample, xdirss, ps, 30.5)

...
def volume_calc(volume, reservoir, numberColumns, overageFactor):
    ...
    if volume > 1800:
        sys.exit("volume for one of the reagents is greater than 1800!")
...
def well_mix(reps, loc, vol, pip):  # mixing on one side of the well
    loc1 = loc.bottom().move(
        types.Point(x=0.7, y=0, z=1.3)
    )  # TODO optimize position units
    loc2 = loc.bottom().move(
        types.Point(x=-0.9, y=0, z=7)
    )  # TODO optimize position units
...
def knock_off(
    reps, loc, vol, pip, xdir
):  # knocks beads off side of wall towards magnet
...
def init_well_mix(
    reps, loc, vol, pip, xdir
):  # mixing on both sides of the well, not used while magnet is engaged!
    if vol > 500:
        zheight = 15
    else:
        zheight = 4
    ...
    loc4 = loc.bottom().move(types.Point(x=-1.3 * xdir, y=-0.8, z=0.3))
    ...
    loc8 = loc.bottom().move(types.Point(x=1 * xdir, y=0, z=0.3))
...
def wash_mix(
    reps, loc, vol, pip
):  # repeat wash where aspirate from left and dispense from right side
...
def supernatant_removal(
    vol, src, dest, pip, xdir, bead_transfer="F", is_waste="F"
):  # magnet is engaged, need to change x depending on odds or evens
    if bead_transfer == "T":
        pip.flow_rate.aspirate = (
            120  # higher flow rate durring transfer of beads to not leave any behind
        )
    else:
    ...
    while tvol > 250:
        pip.aspirate(
            250, src.bottom().move(types.Point(x=-1 * xdir, y=0, z=0.8))
        ...
        if is_waste == "T":
            pip.aspirate(
                5, dest
            )  # presumably aspirating 10 ul to get rid of any drops that are still on the pipette
        else:
            dest
    ...
    if bead_transfer == "T":
        pip.aspirate(
            tvol, src.bottom().move(types.Point(x=0, y=0, z=-0.1))
        )  # aspirate remaining volume
    else:
        pip.aspirate(
            tvol, src.bottom().move(types.Point(x=-1.2 * xdir, y=0, z=0.8))
        )  # aspirate remaining volume
    dvol = (
        30 * asp_ctr + tvol
...
def wash_step(
    sample, src, vol, mtimes, tips, usedtips, pip, zheight, xdir
):  # magnet not engaged
    for well, tip, tret, srcWell in zip(sample, tips, usedtips, src):
        ...
        while tvol > 250:
            ...
            asp_ctr = asp_ctr + 1
        pip.aspirate(
            250, srcWell.bottom().move(types.Point(x=-0, y=0, z=1))
        )  # aspirate remaining volume
        dvol = (
            20 * asp_ctr + 250
        )  # calculate volume that was just aspirated and add to it 20 ul for every time there was pipetting
        ...
        init_well_mix(
            mtimes, well, 200, pip, xdir=xdir
        )  # origionally did only 180...why?
        pip.blow_out()
        pip.drop_tip(tret)  # use for next wash step, can save tips for next step


def wash_step_1_5(
    sample, src, vol, mtimes, tips, usedtips, pip, zheight, xdir, is_etoh="F"
):  # magnet not engaged
    for well, tret in zip(sample, usedtips):
        ...
        if is_etoh == "T":
            ...
            pip.dispense(100, well.bottom().move(types.Point(x=-0, y=-1, z=zheight)))
            pip.aspirate(
                100, well.bottom().move(types.Point(x=-0, y=-1, z=zheight - 4))
            )
            pip.dispense(
                100, well.bottom().move(types.Point(x=-1 * xdir, y=-1, z=zheight + 4))
            )
        pip.drop_tip(tret)  # use for next wash step, can save tips for next step


def wash_step2(
    sample, src, vol, mtimes, tips, usedtips, pip, waste, xdir
):  # 2a returns tips , magnet engaged
    for well, tip in zip(sample, usedtips):
        pip.pick_up_tip(tip)
        supernatant_removal(
            vol + 20, well, waste, pip, xdir, bead_transfer="F", is_waste="T"
        )
        pip.aspirate(
            5, waste
        )  # making sure there is no liquid on the tip when returning the pipete
        pip.drop_tip()
...
TOT_LYSMAG_WITH_45_GAP = (
    2.5  # (2.5 min) total time of one lysis/mag mixing loop including 45sec delay
)
TOT_LYSMAG_WO_45_GAP = (
    1.8  # (1.8 min) total time of one lysis/mag mixing loop with out 45sec delay
)
NUM_LYSMAG_ITER = round(
    TOT_LYSMAG_INC
    / (
        TOT_LYSMAG_WITH_45_GAP
        if numberColumns == 1
        else TOT_LYSMAG_WO_45_GAP * numberColumns
    )
)  # number of loops dependent on number of columns and total time
TOT_ELUTE_INC = 15  # (15 min) total time of mixing loops for elution
TOT_ELUTE_WITH_45_GAP = (
    2.5  # (2.5 min) total time of one lysis/mag mixing loop including 45sec delay
)
TOT_ELUTE_WO_45_GAP = (
    1.8  # (1.8 min) total time of one lysis/mag mixing loop with out 45sec delay
)
NUM_ELUTE_ITER = math.ceil(
    TOT_ELUTE_INC
    / (
        TOT_ELUTE_WITH_45_GAP
        if numberColumns == 1
        else TOT_ELUTE_WO_45_GAP * numberColumns
    )
)  # number of loops dependent on number of columns and total time
...
def run(protocol):
    magdeck = protocol.load_module("magnetic module gen2", "9")
    ...
    reagentPlate = protocol.load_labware(
        "usascientific_96_wellplate_2.4ml_deep", "3", "Reagent Plate"
    )
    print("96 well ethanol plate in slot 8")
    ethanolPlate = protocol.load_labware(
        "usascientific_12_reservoir_22ml", "8", "Ethanol Plate"
    )
    #usascientific_12_reservoir_22ml
    print("96 well DNA elution plate in slot 10")
    elutionPlate = protocol.load_labware(
        "nest_96_wellplate_100ul_pcr_full_skirt", "1", "DNA Elution Plate"
    )
    print("96 well RNA elution plate in slot 4")
    nextRNAPlate = protocol.load_labware(
        "usascientific_96_wellplate_2.4ml_deep", "4", "RNA Elution Plate (DNA Protocol)"
    )
    tips200 = [
    ...
    [tips1, tips2, tips3, tips4, tips5, tips6, tips7, tips8] = [
        all_tips[i : i + numberColumns]
        for i in range(0, numberColumns * 8, numberColumns)
    ]
    s_tips = [
    ...
    wash1, reagentCounter = reagent_assign(
        wash1Volume, reagentPlate, reagentCounterInit
    )
    ...
    sampleDNA = [
        ...
        for i in range(elutionStartwell, elutionStartwell +  numberColumns , 1)
    ]

    sampleRNA = [
        nextRNAPlate["A" + str(i)]
        for i in range(startWell, 2 * numberColumns + startWell, 2)
    ]
    ...
    protocol.pause("Did you remember to change the elutionStartwell number??")
    ...
    wash_step(sample, wash1, 500, 4, tips3, tips3, p300, zheight=10, xdir=xdir)
    magdeck.engage(height=magheight)
    protocol.delay(minutes=2)
    wash_step_1_5(sample, wash1, 500, 5, tips3, tips3, p300, zheight=7, xdir=xdir)
    protocol.delay(minutes=4)
    wash_step2(sample, wash1, 500, 5, tips3, tips3, p300, waste, xdir)
    magdeck.disengage()
    wash_step(sample, wash2, 500, 3, tips4, tips4, p300, zheight=10, xdir=xdir)
    magdeck.engage(height=magheight)
    protocol.delay(minutes=2)
    wash_step_1_5(sample, wash2, 500, 5, tips4, tips4, p300, zheight=7, xdir=xdir)
    protocol.delay(minutes=4)
    wash_step2(sample, wash2, 500, 5, tips4, tips4, p300, waste, xdir)
    magdeck.disengage()
    for well, tip, tret, srcWell, dest in zip(
        sample, tips5, tips5, ethanol1, sampleTransfer
    ):
        p300.pick_up_tip(tip)
        ...
        wash_mix(
            3, well, 200, p300
        )  # added this because there was some left after transfer
        supernatant_removal(
            200, well, dest, p300, xdir=xdir, bead_transfer="T"
        )  # added this because there was some left after transfer
    ...
    protocol.delay(minutes=2)
    wash_step_1_5(
        sample, ethanol1, 600, 5, tips6, tips6, p300, zheight=7, xdir=xdir, is_etoh="T"
    )
    protocol.delay(minutes=4)
    wash_step2(sample, ethanol1, 850, 5, tips6, tips6, p300, waste, xdir=xdir * -1)
    magdeck.disengage()
    wash_step(sample, ethanol2, 1200, 3, tips7, tips7, p300, zheight=15, xdir=xdir * -1)
    magdeck.engage(height=magheight)
    protocol.delay(minutes=2)
    wash_step_1_5(
        sample,
    ...
    protocol.delay(minutes=4)
    wash_step2(sample, ethanol2, 1200, 5, tips7, tips7, p300, waste, xdir=xdir * -1)
    ...
    """
        ...
        """
    protocol.comment("Allowing beads to air dry for 8 minutes.")
    protocol.delay(minutes=4)
    ...
    for src, dest, tip, tret in zip(sample, sampleDNA, tips1, tips1):
        p300.pick_up_tip(tip)
        p300.aspirate(60, src.bottom().move(types.Point(x=-1 * xdir * -1, y=0, z=0.5)))
        p300.dispense(100, dest)
        p300.drop_tip()  # drop in trash
    magdeck.disengage()

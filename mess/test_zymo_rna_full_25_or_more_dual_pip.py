...
def run(protocol):
    ...
    print(unique(lysMag))
    print("\n")
    print("lysis + sheild or WB1 here:")
    ...
    protocol.delay(minutes=.04)  # TODO how much time does it really take!
    ...
    for l_well, r_well, reagent, l_tip, r_tip in zip(
        ...
        p300R.blow_out()
        p300R.aspirate(10, r_well.top(-2))
        p300L.drop_tip()
        p300R.drop_tip()
    i = 1
    ...
    for l_well, r_well, reagent, l_tip, r_tip in zip(
        ...
        p300L.dispense(40, l_well.top(-10))
        init_well_mix(1, l_well, 200, p300L, xdir)
        p300L.aspirate(10, l_well.top())
        p300R.dispense(40, r_well.top(-10))
    ...
    for l_well, r_well, l_tip, r_tip, srcWell, l_dest, r_dest in zip(
        ...
        if dvol > 250:
            dvol = 250
        p300L.dispense(dvol + 10, l_well.top())
        p300R.dispense(dvol + 10, r_well.top())
        p300R.aspirate(10, r_well.top())
    ...
    for _ in range(2):
        for l_well, r_well, l_tip, r_tip in zip(l_sample, r_sample, tips3_L, tips3_R):
            ...
            p300L.return_tip()
            p300R.return_tip()
    protocol.comment("Adding DNA/RNA Prep!")
    wash_step(
        l_sample,
        r_sample,
        dnaRNAPrepBuffer,

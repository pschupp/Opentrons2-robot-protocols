...
metadata = {
    "protocolName": "Twist Exome 2.0 pt2",
...
def p300_supernatant_removal(vol, src, dest, pip, xdir, super_type="sample"):
    ...
    if (
        ...
        pip.flow_rate.aspirate = 5
        pip.aspirate(
            120, src.bottom().move(types.Point(x=-1 * xdir, y=0, z=2.2))
        )  # aspirating away from the side with the magnests
        pip.aspirate(10, src.top())
        pip.dispense(300, dest)
        pip.blow_out()
...
startWell = 1
SPRI_1_vol = 60
SPRI_2_vol = 50  # more for dual index kit
adapter_vol = 5  # Change adapter vol based on DNA input
...
frag_mm_volume = volume_calc(10, "96well100", numberColumns, overageFactor)
print("Adapter Master Mix")
adapt_mm_volume = volume_calc(adapter_vol, "96well100", numberColumns, overageFactor)
print("Ligation Master Mix")
ligation_mm_volume = volume_calc(20, "96well100", numberColumns, overageFactor)
print("Amplification Master Mix")
amp_mm_volume = volume_calc(25, "96well100", numberColumns, overageFactor)
...
def run(protocol):
    ...
    def p300_mastermix_transfer(
        ...
        pip.flow_rate.dispense = 30
        pip.pick_up_tip(tips[0])
        for tip, r_well, s_well, mix_vol in zip(
            tips, reagent_well, sample_well, mix_volume
        ):
            if not pip.hw_pipette["has_tip"]:
                pip.pick_up_tip(tip)
            tvol = transfer_vol
            ...
            if mix_type == "bead_mix":
                ...
                p20_mix(8, mix_vol, s_well, pip, high_mix="T")
            pip.dispense(mix_vol + 10, s_well.top(-8))
            pip.blow_out(s_well.top(-6))
            pip.touch_tip(v_offset=-8)
            pip.drop_tip()
        pip.flow_rate.aspirate = 50  # set rate back to the rate from before
    ...
    def SPRI_clean_up_a(pip, tips, sample, bead_vol, xdirs, mag_bead, total_bead_vol):
        ...
        for tip, beads, mag_samp, xdir in zip(tips, mag_bead, sample, xdirs):
            ...
            if not pip.hw_pipette["has_tip"]:
                pip.pick_up_tip(tip)
            p300_mix(3, 40, beads, pip, xdir=xdir, high_mix="T")
            pip.blow_out()
            pip.flow_rate.aspirate = 5
            pip.flow_rate.dispense = 50
            ...
            pip.flow_rate.dispense = 50
            if pip.name == "p300_multi_gen2":
                p300_mix(8, 60, mag_samp, pip, xdir=xdir)
            elif pip.name == "p20_multi_gen2":
    ...
    def SPRI_clean_up_c(pip, tips, sample, xdirs, elution_buffer, eb_vol):
        ...
        for tip, eb, mag_samp, xdir in zip(tips, elution_buffer, sample, xdirs):
            ...
            if pip.name == "p300_multi_gen2":
                ...
                if eb_vol < 20:
                    p20_mix(12, eb_vol, mag_samp, pip, xdir=xdir)
                else:
    ...
    frag_mm, reagentCounter = reagent_assign(
        frag_mm_volume, mm_plate, reagentCounterInit
    )
    adapt_mm, reagentCounter = reagent_assign(adapt_mm_volume, mm_plate, reagentCounter)
    ligation_mm, reagentCounter = reagent_assign(
        ligation_mm_volume, mm_plate, reagentCounter
    )
    amp_mm, reagentCounter = reagent_assign(amp_mm_volume, mm_plate, reagentCounter)
    ...
    print("\n")
    print("Fragmentation mastermix here:")
    print(frag_mm_volume["reservoir.volume"])
    print(unique(frag_mm))
    print("\n")
    print("Adapter mastermix here:")
    print(adapt_mm_volume["reservoir.volume"])
    print(unique(adapt_mm))
    print("\n")
    print("Ligation mastermix here:")
    print(ligation_mm_volume["reservoir.volume"])
    print(unique(ligation_mm))
    print("\n")
    print("Amplication mastermix here:")
    print(amp_mm_volume["reservoir.volume"])
    print(unique(amp_mm))
    print("\n")
    ...
    protocol.pause("incubation delays have been commented out for testing purposes")
    '''
    # Bead Purification
    magdeck.disengage()

    SPRI_clean_up_a(p300, TIPS_1, mag_sample, 60, xdirss, mag_bead1, 120)
    ...
    '''
    SPRI_clean_up_e(
        p300, [TIPS_2, TIPS_3, TIPS_4, TIPS_5], mag_sample, waste, xdirss, ethanol
    )
    # protocol.delay(minutes=4)  # let beads dry

    magdeck.disengage()

    protocol.pause("Transfer plate to other robot for elution")

    """
    #Elution 

    SPRI_clean_up_c(p300, TIPS_6, mag_sample, xdirss, elution_buffer, 17.5)
    ...
    SPRI_clean_up_f(p20, tips_2, mag_sample, 15.5, warm_sample, xdirss)
    # add to plate with indexes


    p300_mastermix_transfer(p300, TIPS_7, amp_mm, warm_sample, 25.5, 30)
    """
    protocol.comment("Transfer plate to thermocycler")
    protocol.comment("Line 971")
    ...
    SPRI_clean_up_a(p300, TIPS_1, mag_sample, 50, xdirss, mag_bead2, 100)
    ...
    SPRI_clean_up_c(p300, TIPS_6, mag_sample, xdirss, elution_buffer, 22.5)
    #protocol.delay(minutes=2)
    magdeck.engage(magheight)
    #protocol.delay(minutes=4)

    protocol.comment("switch plate to other robot for final elution")
    """
    SPRI_clean_up_f(p20, tips_2, mag_sample, 20, warm_sample, xdirss)
    """
    protocol.comment("Line 1006")
    protocol.comment("Part 4 Done!")
    protocol.comment("On to QC!")

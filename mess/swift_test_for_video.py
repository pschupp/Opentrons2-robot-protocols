...
def SPRI_clean_up_a(
    ...
    for tip, beads, mag_samp, xdir in zip(tips, mag_bead, sample, xdirs):
        if i == 1:
            ...
            pip.flow_rate.dispense = 150
            pip.pick_up_tip(tip)
            p300_mix(2, 220, beads, pip, xdir=xdir)
        if not pip.hw_pipette["has_tip"]:
            pip.pick_up_tip(tip)
        p300_mix(1, 100, beads, pip, xdir=xdir)
        ...
        pip.flow_rate.aspirate = 70
        pip.flow_rate.dispense = 50
        p300_mix(3, 80, mag_samp, pip, xdir=xdir)
...
def run(protocol):
    ...
    rt_reagents = protocol.load_labware("usascientific_96_wellplate_2.4ml_deep", "2")

...
def volume_calc(volume,reservoir,numberColumns,overageFactor):
	...
	if volume>2000: sys.exit('volume for one of the reagents is greater than 2200!')
...
def init_well_mix(reps,loc,vol,pip,xdir,wellVol=499): 
	...
	loc3 = loc.bottom().move(types.Point(x=-1.3*xdir,y=-.8,z=0.3)) 
	loc4 = loc.bottom().move(types.Point(x=-1.2*xdir,y=-.8,z=0.3))
...
def fetch_beads(vol,src,sample,dest,pip,tips,usedtips):
	for srcWell,well,tip,tret,destWell in zip(src,sample,tips,usedtips,dest): 
		...
		pip.aspirate(vol+20,well.bottom().move(types.Point(x=0,y=0,z=-0.2))) 
...
def supernatant_removal(vol,src,dest,pip,xdir,bead_transfer='F',is_waste='F'): 
	...
	asp_ctr = 0 
	while tvol > 250:
		pip.aspirate(250,src.bottom().move(types.Point( x=-1.3*xdir,y=0,z=.8))) 
	...
	if bead_transfer=='T':
		...
		pip.aspirate(tvol,src.bottom().move(types.Point(x=-1.3*xdir,y=0,z=.8))) 
...
def supernatant_removal_mini(vol,src,dest,pip,xdir):
	pip.pick_up_tip()
	pip.aspirate(15,src.bottom().move(types.Point(x=-0.3*xdir,y=0,z=.4)))
...
def wash_step_1_5(sample,src,vol,mtimes,tips,usedtips,pip,zheight,is_etoh='F') :	
	for sampleWell,tret in zip(sample,usedtips): 
		...
		if is_etoh== 'T':
			...
			pip.aspirate(100,sampleWell.bottom().move(types.Point(x=-0,y=1,z=zheight))) 
			pip.dispense(100,sampleWell.bottom().move(types.Point(x=-0,y=1,z=zheight))) 
			pip.aspirate(100,sampleWell.bottom().move(types.Point(x=-0,y=-1,z=zheight))) 
			pip.dispense(100,sampleWell.bottom().move(types.Point(x=-0,y=-1,z=zheight))) 
			pip.aspirate(100,sampleWell.bottom().move(types.Point(x=-0,y=1,z=zheight))) 
			pip.dispense(100,sampleWell.bottom().move(types.Point(x=-0,y=1,z=zheight))) 
			pip.aspirate(100,sampleWell.bottom().move(types.Point(x=-0,y=-1,z=zheight-4))) 
			pip.dispense(100,sampleWell.bottom().move(types.Point(x=-0,y=-1,z=zheight+4))) 
		pip.drop_tip(tret) 
def wash_step2(sample,src,vol,mtimes,tips,usedtips,pip,waste,xdir,return_tips='F') : 
...
ethanol0Volume=volume_calc(700,"96well",numberColumns,overageFactor)
print("Ethanol Buffer 1")
ethanol1Volume=volume_calc(1000,"96well",numberColumns,overageFactor)
print("Ethanol Buffer 2")
ethanol2Volume=volume_calc(1000,"96well",numberColumns,overageFactor)
print("Ethanol Buffer 3")
ethanol3Volume=volume_calc(600,"96well",numberColumns,overageFactor)
print("Ethanol Buffer 4")
ethanol4Volume=volume_calc(1200,"96well",numberColumns,overageFactor)
...
def run(protocol):
	...
	reagentPlate=protocol.load_labware('nest_96_wellplate_2ml_deep','2','Reagent Plate') 
	ethanolPlate=protocol.load_labware('usascientific_12_reservoir_22ml','3','Ethanol Plate') 
	ethanolPlate2=protocol.load_labware('usascientific_12_reservoir_22ml','4','Ethanol Plate 2') 
	print('reagent 96-well plate in slot 3')
	elutionPlate=protocol.load_labware('usascientific_96_wellplate_2.4ml_deep','10','Elution Plate') 
	...
	[tips1,tips2,tips3,tips4,tips5,tips6,tips7,tips8,tips9,tips10] = [all_tips[i:i+numberColumns] for i in range(0,numberColumns*10,numberColumns) ] 
	...
	magBead,reagentCounter=reagent_assign(magBeadVolume,reagentPlate,reagentCounterInit)
	...
	ethanol0,reagentCounter=reagent_assign(ethanol0Volume,reagentPlate,reagentCounter)
	...
	ethanol3,ethanolCounter2=reagent_assign(ethanol3Volume,ethanolPlate2,ethanolCounterInit2)
	ethanol4,ethanolCounter2=reagent_assign(ethanol4Volume,ethanolPlate2,ethanolCounter2)
	dnaseIReactionMix,reagentCounter=reagent_assign(dnaseIReactionMixVolume,reagentPlate,reagentCounter)
	dnaRNAPrepBuffer,reagentCounter=reagent_assign(dnaRNAPrepBufferVolume,reagentPlate,reagentCounter)
	...
	print(sample)
	sampleTransfer1=[magplate['A'+str(i)] for i in range(startWell+1,startWell+numberColumns*2+1,2)]
	print(sampleTransfer1)
	sampleTransfer2=[elutionPlate['A'+str(i)] for i in range(startWell+1,startWell+numberColumns*2+1,2)]
	print(sampleTransfer2)
	sampleRNA=[elutionPlate['A'+str(i)] for i in range(startWell,startWell+numberColumns*2,2)]
	print(sampleRNA)
	print('\n')
	print('Mag bead here:')
	...
	magdeck.disengage()
	for well,reagent,tip in zip(sample,ethanol0,tips1): 
		...
		init_well_mix(3,well,250,p300,xdir,1400) 
		p300.blow_out()
	...
	if volumeMix>280: 
		volumeMix=280
	for well,reagent,tip in zip(sample,magBead,tips2): 
		p300.pick_up_tip(tip)
		if i==1: 
			init_well_mix(2,reagent,volumeMix,p300,xdir) 
		init_well_mix(3,reagent,volumeMix,p300,xdir=xdir) 
...
#	protocol.delay(minutes=31) 
	...
	for well,tip,tret in zip(sample,tips2,tips2): 
		p300.pick_up_tip(tip)
	...
	magdeck.engage(height=magheight)
	protocol.delay(minutes=3)
	wash_step_1_5(sample,wash1,500,5,tips4,tips4,p300,zheight=7)
	protocol.delay(minutes=4)
	wash_step2(sample,wash1,500,5,tips4,tips4,p300,waste,xdir) 
	magdeck.disengage()
	wash_step(sample,wash2,500,4,tips5,tips5,p300,zheight=10,xdir=xdir)
	magdeck.engage(height=magheight)
	protocol.delay(minutes=3)
	wash_step_1_5(sample,wash2,500,5,tips5,tips5,p300,zheight=7)
	protocol.delay(minutes=4)
	wash_step2(sample,wash2,500,5,tips5,tips5,p300,waste,xdir)
	magdeck.disengage()
	wash_step(sample,ethanol1,1000,4,tips6,tips6,p300,zheight=15,xdir=xdir) 
	magdeck.engage(height=magheight)
	protocol.delay(minutes=3)
	wash_step_1_5(sample,ethanol1,1000,5,tips6,tips6,p300,zheight=15,is_etoh='T')
	protocol.delay(minutes=4)
	wash_step2(sample,ethanol1,500,5,tips6,tips6,p300,waste,xdir=xdir,return_tips='T') 
	wash_step_1_5(sample,ethanol1,500,5,tips6,tips6,p300,zheight=15,is_etoh='T')
	wash_step2(sample,ethanol1,500,5,tips6,tips6,p300,waste,xdir=xdir) 
	magdeck.disengage()
	for well,tip,tret,srcWell,dest in zip(sample,tips7,tips7,ethanol2,sampleTransfer1): 
		p300.pick_up_tip(tip) 
		tvol = 600 
		asp_ctr = 0 
		while tvol > 250: 
		...
		p300.blow_out() 
		p300.drop_tip()
	fetch_beads(200,ethanol2,sample,sampleTransfer1,p300,tips8,tips8)
	
	sample=sampleTransfer1
	
	magdeck.engage(height=magheight)
	protocol.delay(minutes=3)
	wash_step_1_5(sample,ethanol2,600,5,tips8,tips8,p300,zheight=15,is_etoh='T')
	protocol.delay(minutes=4)
	wash_step2(sample,ethanol2,850,5,tips8,tips8,p300,waste,xdir=xdir*-1)
	magdeck.disengage()
	for well,reagent,tip,tret in zip(sample,dnaseIReactionMix,tips9,tips9): 
		p300.pick_up_tip(tip)
		p300.aspirate(50,reagent) 
		p300.dispense(80,well.top(-20))
		elute_mix(4,well,45,p300,xdir=xdir*-1)
		p300.drop_tip(tret)	
	for _ in range(3):
		for well,tip,tret in zip(sample,tips9,tips9):	
			p300.pick_up_tip(tip)
			elute_mix(3,well,45,p300,xdir=xdir*-1) 
			p300.drop_tip(tret)	
	wash_step(sample,dnaRNAPrepBuffer,500,2,tips10,tips10,p300,zheight=10,xdir=xdir*-1) 	
#	protocol.delay(minutes=11) 
	...
	wash_step_1_5(sample,dnaRNAPrepBuffer,500,5,tips10,tips10,p300,zheight=7) 
	protocol.delay(minutes=4)
	wash_step2(sample,dnaRNAPrepBuffer,600,5,tips10,tips10,p300,waste,xdir=xdir*-1)
	magdeck.disengage()
	for well,tip,tret,srcWell,dest in zip(sample,tips1,tips1,ethanol3,sampleTransfer2): 
		p300.pick_up_tip(tip) 
		tvol = 600 
		asp_ctr = 0 
		while tvol > 250: 
	...
	fetch_beads(200,ethanol2,sample,sampleTransfer2,p300,tips2,tips2)
	magdeck.engage(height=magheight)
	protocol.pause("Move sample to magnet!")
	protocol.delay(minutes=3)
	wash_step_1_5(sample,ethanol3,600,5,tips2,tips2,p300,zheight=7,is_etoh='T')
	protocol.delay(minutes=4)
	wash_step2(sample,ethanol3,850,5,tips2,tips2,p300,waste,xdir=xdir*-1) 
	magdeck.disengage()
	protocol.comment('!!!!!!!!REPLACE TIPS 1,2,AND 3!!!!!!')
	wash_step(sample,ethanol4,1000,3,tips3,tips3,p300,zheight=15,xdir=xdir*-1)
	magdeck.engage(height=magheight)
	protocol.delay(minutes=3)
	wash_step_1_5(sample,ethanol4,1000,5,tips3,tips3,p300,zheight=15,is_etoh='T')
	protocol.delay(minutes=4)
	wash_step2(sample,ethanol4,700,5,tips3,tips3,p300,waste,xdir=xdir*-1,return_tips='T')
	wash_step_1_5(sample,ethanol4,700,5,tips3,tips3,p300,zheight=15,is_etoh='T')
	wash_step2(sample,ethanol4,500,5,tips3,tips3,p300,waste,xdir=xdir*-1,return_tips='T')
	protocol.comment('Allowing beads to air dry for 5 minutes.')
	protocol.delay(minutes=5)
	for src,tip in zip(sample,tips3):
		p300.pick_up_tip(tip)
		p300.aspirate(65,src.bottom().move(types.Point(x=1,y=0,z=.5))) 
		p300.dispense(100,waste)
		p300.drop_tip()  
	protocol.delay(minutes=7)
	magdeck.disengage()
	protocol.comment('Adding NF-Water to wells for elution:')
	for well,tip,tret,waterWell in zip(sample,tips4,tips4,water): 
		p300.pick_up_tip(tip)
...
#	protocol.delay(minutes=11) 
	...
	for well,tip,tret in zip(sample,tips4,tips4): 
		p300.pick_up_tip(tip) 
	...
	for src,dest,tip in zip(sample,sampleRNA,tips5):
		p300.pick_up_tip(tip)
		p300.aspirate(60,src.bottom().move(types.Point(x=1,y=0,z=.5))) 

...
ethanol4Volume = volume_calc(1200, "seperate_96well", numberColumns, overageFactor)
...
def run(protocol):
    ...
    ethanolPlate2 = protocol.load_labware(
        "usascientific_12_reservoir_22ml", "4", "Ethanol Plate 2"
    )
    print("reagent 96-well plate in slot 3")
    elutionPlate = protocol.load_labware(
        "nest_96_wellplate_2ml_deep", "10", "Elution Plate"
    )
    ...
    ethanol0, ethanolCounter2 = reagent_assign(
        ethanol0Volume, reagentPlate, ethanolCounterInit2
    )
    ...
    ethanol3, ethanolCounter2 = reagent_assign(
        ethanol3Volume, ethanolPlate2, ethanolCounter2
    )
    ethanol4, ethanolCounter2 = reagent_assign(
        ethanol4Volume, ethanolPlate2, ethanolCounter2
    )
    dnaseIReactionMix, reagentCounter = reagent_assign(
        dnaseIReactionMixVolume, reagentPlate, reagentCounter
    )
    dnaRNAPrepBuffer, reagentCounter = reagent_assign(
        dnaRNAPrepBufferVolume, reagentPlate, reagentCounter
    )
    ...
    for well, reagent, tip in zip(sample, lysMag, tips1):  # TODO measure time
        ...
        if i in range(
            ...
            init_well_mix(3, reagent, 220, p300, xdir=xdir)
